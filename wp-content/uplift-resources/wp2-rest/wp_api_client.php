...
class WP_API_Client
{
    ...
    const ENDPOINT_POSTS = '/wp-json/wp/v2/posts';
    ...
    {
        return $this->get($this->build_url(self::ENDPOINT_USERS, array('filter' => $filters, 
                                                                       'context' => $context, 
                                                                       'page' => $page)));
    }

    /**
     * Retrieve a subset of the site's posts.
     *
     * @param  array   $filters
     * @param  integer $perPage  Number of posts returned per page 
     * @param  integer $page     Page number for above
     * @param  string  $postID   For retreiving specific posts
     ...
     * @return array|WP_Error
     */
    public function get_posts(array $filters = array(), $perPage = 10, $page = 1, $postID = '', $orderby = 'date', $order = 'desc', $context = 'view')
    {
        $args = array('filter' => $filters,
                      'context' => $context,
                      'per_page' => $perPage, 
                      'page' => $page,
        ...
        return $this->get($this->build_url(self::ENDPOINT_POSTS, $args));
    }

    /**
     * Retrieve a single post. This simply calls get_posts()
     *
     * @param  string   $postIDs   Comma separated List of Post IDs
     * @param  integer  $numPosts  The number of posts to return - generally corresponds to number of posts in the list of IDs
     * @param  string  $orderby  Order by specific field
     * @param  string  $order    Ascending or descending
     *
     * @return array|WP_Error
     */
    public function get_post($postIDs, $numPosts = 1, $orderby = 'date', $order = 'desc')
    {
        return $this->get_posts(array(), $numPosts, 1, $postIDs,  $orderby, $order, 'view');      
    }
